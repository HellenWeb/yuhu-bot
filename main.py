...
async def cart(message: types.Message):
    if mongo.show_history(message.from_user.id):
        ...
        for i in mongo.show_history(message.from_user.id):
            ...
            mark3.add(
                types.InlineKeyboardButton(
                    text=f"❌", callback_data=f"delete {i['product']}"
                )
            )
            await message.answer(
                f"<strong>Название: </strong>{i['product']}\n<strong>Описание: </strong>{i['description']}\n<strong>Цена:</strong> {i['price']}",
                reply_markup=mark3,
            )
            result += int(i["price"])
            count += 1
        ...
        mark2.row(f"🏠")
        await message.answer(
            f"Корзина ({count}) & Общая сумма <strong>{result} руб.</strong>",
            reply_markup=mark2,
            parse_mode="html",
        )
    else:
...
async def catalog(c: types.CallbackQuery):
    ...
    for i in mongo.show_categories():
        if c.data == i["title"]:
            try:
                ...
                for n in mongo.show_under_categories(i["title"]):
                    mark1.row(
                        types.InlineKeyboardButton(
                            text=n["title"], callback_data=n["title"]
                        )
                    )
            ...
            if c.data == n["title"]:
                ...
                for f in mongo.show_product(n["title"]):
                    try:
                        ...
                        mark2.add(
                            types.InlineKeyboardButton(
                                text=f"Добавить в корзину 🛍", callback_data=f["title"]
                            )
                        )
                        mark2.add(
                            types.InlineKeyboardButton(
                                text=f'Купить - {f["price"]} руб.',
                                callback_data=f["price"],
                            )
                        )
                        await c.message.answer_photo(pic)
                        await c.message.answer(
                            f'<strong>Название: </strong>{f["title"]}\n<strong>Описание: </strong>{f["description"]}',
                            reply_markup=mark2,
                        )
                        count += 1
                ...
                mark3.row("🏠", "🛒", "🛍")
                await c.message.answer(
                    f"Позаны все товары ({count})", reply_markup=mark3
                )
        ...
        for n in mongo.show_under_categories(i["title"]):
            for f in mongo.show_product(n["title"]):
                if c.data == f["title"]:
                    try:
                        mongo.inster_history(
                            c.from_user.id, f["title"], f["description"], f["price"]
                        )
                        await c.message.answer(
    ...
    for p in mongo.show_history(c.from_user.id):
        if c.data == f'delete {p["product"]}':
            try:
                ...
                await c.message.answer(
                    f'Товар успешно удален (<strong>{p["product"]}</strong>) ❎',
                    parse_mode="html",
                )
            except IndexError:
    ...
    if c.data == "back":
        ...
        for i in mongo.show_categories():
            mark1.row(
                types.InlineKeyboardButton(text=i["title"], callback_data=i["title"])
            )
        mark1.add(types.InlineKeyboardButton(text="🏠", callback_data="home"))
        await bot.edit_message_text(
            chat_id=c.message.chat.id,
    ...
    if c.data == "home":
        ...
        mark1.row("Корзина 🛍", "О нас ❕")
        await c.message.answer(
            f"Привет {c.message.from_user.first_name} 😁\n<strong>Это телеграмм бот магазина ЮХУ</strong>\nЗдесь ты сможешь купить нужный тебе товар и забрать его в ближайшем нашем магазине\nCreator: @YungHellen",
            parse_mode="html",
            reply_markup=mark1,
        )
...
async def keyboardbutton(message: types.Message):
    if message.chat.type == "private":
        if message.text == "Помощь ❓":
            await message.answer(
                f"/start или /help - <strong>Главное меню</strong>\n/settings - <strong>Настройки</strong>\n/catalog - <strong>Каталог</strong>\n/about - <strong>О нас</strong>\n/cart - <strong>Корзина</strong>\n/history - <strong>История покупок</strong>\n\nВыберите ниже раздел справки и получите краткую помощь. Если Ваш вопрос не решен, обратитесь за помощью к живому оператору @YungHellen.",
                parse_mode="html",
            )
        ...
        if message.text == "Каталог 🛒":
            ...
            for i in mongo.show_categories():
                mark1.row(
                    types.InlineKeyboardButton(
                        text=i["title"], callback_data=i["title"]
                    )
                )
            mark1.add(types.InlineKeyboardButton(text="🏠", callback_data="home"))
            await message.answer(f"Категории товаров:", reply_markup=mark1)
        ...
        if message.text == "Купить всё":
            try:
                if int(mongo.show_users(message.from_user.id)["age"]) < 18:
                    await message.answer(
                        "Лицам младше 18 лет продажа тобачной продукции запрещена ⛔️"
                    )
                else:
                    for i in mongo.show_history(message.from_user.id):
                        mongo.inster_cart(
                            message.from_user.id,
                            mongo.show_users(message.from_user.id)["name"],
                            mongo.show_users(message.from_user.id)["number"],
                            mongo.show_users(message.from_user.id)["age"],
                            i["product"],
                            i["price"],
                        )
